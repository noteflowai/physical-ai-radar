...
| | **このレーダー** | よくある情報源 |
| --- | --- | --- |
| **更⁠新** | 毎日 07:40 Asia/Singapore、自動 | ニュースレターは週次か不定期 · awesome リストは貢献次第 · arXiv は毎日 |
| **範⁠囲** | 論文 + 公式 + 報道、8 つの軸 | ニュースレターは編集者の選定 · awesome はテーマ別に蓄積 · arXiv は論文のみ全件 |
| **根⁠拠** | 全項目に `O` / `R` / `M` | 通常なし |
| **数⁠値** | 自動抽出、原文の文脈つき | 著者次第、または自分で読む |
| **言⁠語** | 中国語・英語・日本語をそれぞれ書き下ろし | 通常は英語のみ |
| **形⁠式** | Atom × 3 · JSON Feed · `latest.json` | メール · Markdown · arXiv の RSS / API |
| **再⁠現** | ルールと重みはすべてリポジトリに、オフラインで再実行可 | 公開されることは少ない |
...
| 軸 | 追跡対象 → 独立させる理由 |
| --- | --- |
| **基盤モデル（VLA / WAM）** | 視覚言語行動モデル、世界行動モデル、公開重み<br>→ 下流タスクの出発点と機体横断の転移可否を決める |
| **データエンジン** | 遠隔操作、一人称人間動画、精査、スケーリング<br>→ 新規タスクあたりのデータコストを決める |
| **学習と自己改善** | RL 事後学習、advantage conditioning、機群規模の循環<br>→ 配備後も方策が強くなれるかを決める |
| **シミュレーションと評価** | sim2real、real-to-sim、閉ループ成功率、評価基盤<br>→ オフライン指標は閉ループ成功率ではない |
| **エッジとリアルタイム** | オンデバイス推論、量子化、非同期推論、制御周期<br>→ 実時間予算を超えるモデルは制御ループに入れない |
| **安全・権限・規制** | セーフティフィルタ、CBF、ISO と規制日程<br>→ 認証の空白期間では、安全は証明書でなく設計に依存する |
| **システムとオーケストレーション** | 長期タスク、オーケストレーター、記憶、方策ルーティング<br>→ 運用対象が単一モデルから能力境界の集合になる |
| **ハードウェアとサプライチェーン** | ヒューマノイド、アクチュエータ、触覚、BOM と生産ライン<br>→ コスト曲線と納入ペースの実際の律速 |
